-- Allow professor exam submissions to be queried globally or by a specific course.
-- The global inbox should surface submissions from every course the reviewer can access.
...
BEGIN
  ...
  IF current_role IS NULL OR current_role NOT IN ('professor', 'admin') THEN
    RAISE EXCEPTION 'Only professors and admins can view submissions';
  END IF;
  ...
  JOIN professor_courses pc ON pc.id = eq.course_id
  JOIN users u ON ua.user_id = u.id
  WHERE (p_course_id IS NULL OR eq.course_id = p_course_id)
